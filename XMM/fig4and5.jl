...
results = load_object("../results/xmm_full_mcmc.jld2")
c, r, σ0, P0, w_all, g_result, β_all, ϕ_all, Σ_all, ind_all  = results
...
for t in 1:T
    if t % 10 == 1
        global idx
        f1s_thinned[idx] = f1s[t]
        f2s_thinned[idx] = f2s[t]
        w_thinned[:, idx] = w_all[:, t]
        idx += 1
    end
end
...
savefig("../results/fig4.pdf", bbox_inches="tight", pad_inches=0.05)
...
fig.text(0.23, 0.05, "CDF Comparison at five X locations",
    ha="center", va="center", fontsize=12)
savefig("../results/fig5.pdf", bbox_inches="tight", pad_inches=0.05)
